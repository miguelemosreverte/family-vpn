...
func (c *VPNClient) handleWebSocketMessages() {
	...
	for c.enabled {
		...
		if msgType == "signal" {
			...
			// Layer 0 Update Protocol: component-aware updates
			...
			if payload != nil {
				...
				log.Printf("[L0] Update received: domain=%s action=%s commit=%s", domain, action, commit[:8])
				log.Printf("[L0] Triggering component-aware auto-update...")

				// Run auto-update script (now component-aware)
				go func() {
					cmd := exec.Command("bash", "client/auto-update.sh")
					cmd.Dir = filepath.Dir(filepath.Dir(os.Args[0])) // Go up to repo root
					output, err := cmd.CombinedOutput()
					if err != nil {
						log.Printf("[L0] Auto-update failed: %v\nOutput: %s", err, output)
					} else {
						log.Printf("[L0] Auto-update success:\n%s", output)
					}
				}()
			}
		} else if msgType == "update_available" {
			// Legacy update protocol (backward compatibility)
